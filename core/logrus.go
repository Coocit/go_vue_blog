...
// Format 实现Formatter(entry *logrus.Entry) ([]byte, errir)接口
func (t *LogFormatter) Format(entry *logrus.Entry) ([]byte, error) {
	...
	//	自定义日期格式
	timetamp := entry.Time.Format("2006-01-02 15:04:05")
	if entry.HasCaller() {
		//	自定义文件路径
		funcVal := entry.Caller.Function
		fileVal := fmt.Sprintf("%s:%d", path.Base(entry.Caller.File), entry.Caller.Line)
		//	自定义输出格式
		fmt.Fprintf(b, "%s[%s] \x1b[%dm[%s]\x1b[0m %s %s %s\n", log.Prefix, timetamp, levelColor, entry.Level, fileVal, funcVal, entry.Message)

	} else {
		fmt.Fprintf(b, "%s[%s] \x1b[%dm[%s]\x1b[0m %s\n", log.Prefix, timetamp, levelColor, entry.Level, entry.Message)
	}
	return b.Bytes(), nil
}
